...
Orchestrates the full video pipeline in two passes:
  ...
  Render pass (once per language, using shared beats):
    ...
    6. Shorts cutter     — group beats into Short segments from audio breakpoints.
    7. Subtitles         — standard (main) + karaoke (Shorts) from Whisper timestamps.
...
from app.agents.agent5_video.services.flux_generator import generate_all_beat_images
...
from app.agents.agent5_video.services.renderer import (
...
def _process_language(
    ...
    Args:
        beats: Beat dicts with media_url from the visual pass (already Flux-generated).
    ...
    logger.info(
        ...
        "beats=%d audio_duration_ms=%d",
        language, content_id, len(beats), audio.duration_ms,
    )
    ...
    for short in shorts:
        path = build_short_props(
            ...
            channel_style=channel_style,
            channel_color_grade=channel_color_grade,
            rehook_paths=audio.short_rehook_paths,
            bridge_paths=audio.short_bridge_paths,
        )
        short_props_pairs.append((short, path))

    # ── PRE_RENDER_ASSET_AUDIT — local images only ────────────────────────────
    _remote_in_main = _audit_props_for_remote_urls(main_props_path, "main")
    _remote_in_shorts: list[str] = []
    for _, sp_path in short_props_pairs:
        _remote_in_shorts.extend(_audit_props_for_remote_urls(sp_path, "short"))
    _all_remote = _remote_in_main + _remote_in_shorts
...
def _audit_props_for_remote_urls(props_path: str, label: str) -> list[str]:
    ...
    for short in props.get("shorts") or []:
        _scan_sections(short.get("sections") or [])
...
def _render_from_existing_props(
    ...
    """Render main + all shorts from props files already on disk.
    ...
    for sp_path in short_prop_files:
        ...
        if settings.verify_renders:
            issues = verify_render(
                mp4_path=short_result["file_path"],
                expected_duration_ms=None,
                fmt="short",
            )
            if issues:
                raise VerifyFailedError(
                    f"Short {short_index} (existing-props) verification failed "
                    f"for language={language}: {issues}"
                )
        db.add(VideoRender(
            content_id=content_id,
            language=language,
...
def _run_renders(
    ...
    """Render main video + all shorts, verifying each before saving VideoRender rows.
    ...
    for short, props_path in short_props_pairs:
        ...
        if settings.verify_renders:
            ...
            if short_issues:
                raise VerifyFailedError(
                    f"Short {short['short_index']} verification failed "
                    f"for language={language}: {short_issues}"
                )
    ...
    logger.info(
        "language=%s done: 1 main + %d short(s) for content %s render_mode=%s",
