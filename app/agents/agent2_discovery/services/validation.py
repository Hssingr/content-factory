...
async def handle_telegram_update(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        if content.status == "AWAITING_MANUAL_STORY":
            ...
            # Story-level feedback: reject this story and re-dispatch discovery
            # with the feedback threaded into the exclusion context.
...
def _handle_change(
    ...
    if validation.revision_count >= max_revisions:
        # Operator has rejected max_revisions stories in a row — apply the
        # channel's configured limit policy to the current one instead of
        # searching forever.
    ...
    db.commit()
    logger.info(
        "STORY_FEEDBACK_REDISCOVERY content=%s channel=%s revision=%d — story "
        "rejected by operator feedback; re-dispatching discovery",
        content.id, channel.id, validation.revision_count,
    )

    # Same avoid-direct-import dispatch pattern as _handle_manual_story_input.
    celery_app.send_task(
        "app.scheduler.tasks.run_agent2_for_channel",
        kwargs={
            "channel_id":       str(channel.id),
    ...
    if not user or not user.telegram_chat_id:
        return None
    message = (
        "🔄 *Story rejected — searching for a different one.*\n\n"
