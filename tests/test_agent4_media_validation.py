...
class TestAgent4MediaValidation(unittest.TestCase):
    def _run(self, tmp, sections, content_id=None):
        ...
        with patch("app.agents.agent4_visuals.services.media_validation.settings.media_path", tmp), \
            ...
            return validate_visual_media_assets(content_id, db)
    ...
    def test_text_card_allowed_without_image(self):
        with TemporaryDirectory() as tmp:
            content_id = uuid.uuid4()
            result = self._run(
                tmp,
        ...
        self.assertEqual(result.warnings[0].code, "text_card_without_local_image")

    def test_text_card_sentinel_allowed(self):
        with TemporaryDirectory() as tmp:
            content_id = uuid.uuid4()
